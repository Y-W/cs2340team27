...
public class Users {
	...
	/*
	 * public void initialize(ArrayList<User> users, User current) { list =
	 * users; currentUser = current; }
	 */
	/**
	 * Getter
	 * 
	 ...
	 *             won't throw this exception
	 */
	public List<Item> getPotentialMatch() throws IOException,
			ClassNotFoundException, ParseException {
		List<Item> match = new LinkedList<Item>();
		if (currentUser == null)
			return match;
		List<Item> lost = Items.getInstance().getLost(currentUser);
		List<Item> found = Items.getInstance().filter(null, null,
